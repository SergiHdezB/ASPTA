...
M = 2; %Receptors
h_expectation = [1 0 0 0;-1 0 0 0]; %Channel init, if either M or L are changed, the dimensions of this
% have to be consistent (h -> Mx(L+1) vector)
N_ESTIMATIONS = 1; %Number of channel estimations
...
estimations = 1;
errors = zeros(N_ESTIMATIONS,1); %Vector of errors of each estimation (for each channel realitzation)
while(estimations <= N_ESTIMATIONS)
    
    %Estimation parameters
beta_expectation = 1;
H_expectationMdim = zeros(D,D+L);
h_variance = zeros(M*(L+1));
h_variance_trace = eye(L+1);
...
for l = 0:L
    decay(l+1) = exp(w*l);
    h(:,l+1) = normrnd(mu_h,(decay(l+1)^-1)*sigma_h,[M,1]);
end
...
for i = 0:L
    for j = 0:L 
        H_variance = ((J^i)')*(F')*F*(J^j)*h_variance_trace(i+1,j+1)...
                     + H_variance;
    end
end
H_variance
...
h_expectation = reshape(h_expectation,L+1,M)' %Channel estimation
h
...
for m = 0:L
    ...
    for n = 0:L
        c = M*n + 1;
        h_variance(k:(k+M-1),c:(c+M-1)) = (h_expectation(:,m+1)')*h_expectation(:,n+1);
    end     
end

h_variance = Ph^-1 + h_variance;

for l = 0:L
    k = M*l + 1;
    for m = 0:L
        c = M*m + 1;
        h_variance_trace(l+1,m+1) = trace(h_variance(k:(k+M-1),c:(c+M-1)));
    end
end
h_variance_trace
